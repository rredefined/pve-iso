...
const PlanCard = ({ name, price, features, popular, href }) => (
  <div
    className={`p-4 sm:p-6 rounded-lg border ${popular ? "border-brand-400" : "border-gray-800"} bg-slate-900 flex flex-col h-full`}
  >
    {popular && (
      <span className="bg-brand-400 ml-auto text-black text-xs font-bold px-3 py-1 rounded-full w-fit mb-4">
        Most Popular
      </span>
    )}
    ...
    <a href={href}>
      <GlowingButton
        className={`w-full py-2 rounded-md text-sm sm:text-base font-medium transition-colors bg-brand-400 text-black hover:bg-brand-500`}
      >
        Select Plan
      </GlowingButton>
    </a>
...
const CpanelHostingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <Helmet>
        <title>cPanel Hosting | Renderbyte</title>
      </Helmet>
      {/* Hero Section */}
